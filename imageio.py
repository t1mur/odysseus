...
def imgimport_intelligent(img_name):
    ...
    **Notes**
    ...
    if not imglist:
        ...
        return imglist[0]
    elif len(imglist)==3 or len(imglist)==4:
        # make an array from the list of frames, with shape (img[0], img[1], 3)
        img_array = np.dstack(imglist)
    elif len(imglist)==4:
        # swap pwoa and pwa
        img_array = np.dstack([imglist[1], imglist[0], imglist[2], imglist[3]])
    elif len(imglist)==6:
        # get rid of first two frames, they're junk
        #modified by cc to keep first, third, and fifth frame
        img_array = np.dstack([imglist[0], imglist[1], imglist[4]])  #edited by cc 9/17/2010
        # img_array = np.dstack(imglist[2:])   #original line
    else:
