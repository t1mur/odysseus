...
pixsize = 20e-6 # 20 um/pixel for old PI camera
#pixsize = 6.45e-6 # 6.45 um/pixel for Coolsnap camera
magnification = 2.0
pixcal = pixsize/magnification
...
def generate_report(rawframes, transimg, img_name, showpdf=True):
    ...
    **Inputs**
    ...
    ToverTF, N, com, ans, rcoord, od_prof, fit_prof = fit_img(transimg, \
            showfig=False, elliptic=(ellip, 0), full_output='achilles')
    # save the fit figure
    ...
    fig2 = plot_several_temp(rcoord[ind-istep:ind+istep], od_prof[ind-istep:ind+istep], \
                            fit_prof[ind-istep:ind+istep], ans, ans_gaussian, ans2)
...
def fit_several_fixedT(rcoord, od_prof, od_cutoff, guess, ToverTF):
    ...
    **Outputs**
    ...
    u, uu, ans_gaussian = do_fit(rcoord, od_prof, od_cutoff, guess, \
                                  fitfunc='gaussian')
    ans2 = []
    temps = np.linspace(0.01, 2, 7)*ToverTF
    for temp in temps:
        u, uu, ans2_temp = do_fit(rcoord, od_prof, od_cutoff, guess, \
                                  fitfunc='idealfermi_fixedT', T=temp)
        ans2.append(ans2_temp)
    return (ans_gaussian, ans2, temps)


def plot_several_temp(rcoord, od_prof, fit_prof, ans, ans_gaussian, ans2):
